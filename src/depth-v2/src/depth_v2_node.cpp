...
void imageToFloatNCHW(cv::Mat inputImage, void *destinationBuffer) {
  ...
  for (int i = 0; i < 3; ++i) {
    auto status = cudaMemcpy(static_cast<char *>(destinationBuffer) +
                                 i * 640 * 512 * sizeof(float32_t),
                             floatImage.data, 640 * 512 * sizeof(float32_t),
...
nvinfer1::ICudaEngine *loadEngine(const std::string &engine_file,
...
class FoundationStereoDepthNode : public rclcpp::Node {
  ...
  // void getCamIntrinsicsFromYaml(const YAML::Node camIntrinsicsYaml, cv::Mat
  // &cameraIntrinsics) {
  ...
  // }
  //
  // void getCamDistortionCoeffs(const YAML::Node camIntrinsicsYaml, cv::Mat
  // &cameraDistCoeffs) {
  //     const auto &dist_coeffs =
  //     camIntrinsicsYaml["cam0"]["distortion_coeffs"];
  //
  ...
  cv::Mat K1, K2, D1, D2, R, T, R1, R2, P1, P2, Q, leftMapX, leftMapY,
      rightMapX, rightMapY;

  // void getDepth(cv::Mat &disparity) {
  //     cv::Mat depth;
  //     cv::divide(cv::Mat(), disparity, depth, cameraIntrinsics.at<float>(0,
  //     0) * 0.24);
  // }

public:
  FoundationStereoDepthNode() : Node("foundation_stereo_depth") {
    ...
    // std::string left_cam_intrinsics_file =
    // this->get_parameter("left_cam_intrinsics_file").as_string(); std::string
    // right_cam_intrinsics_file =
    // this->get_parameter("right_cam_intrinsics_file").as_string(); if
    // (left_cam_intrinsics_file.empty() || right_cam_intrinsics_file.empty()) {
    // RCLCPP_ERROR(this->get_logger(), "camera intrinsics file not provided!");
    // return;
    // }
    K1 = (cv::Mat_<double>(3, 3) << 405.57512495273403, 0.0, 313.2365778080362,
          0.0, 405.5592192044737, 237.85962939282086, 0.0, 0.0, 1.0);

    K2 = (cv::Mat_<double>(3, 3) << 402.67129522383914, 0, 311.84481417550376,
          0, 402.4522895606074, 241.23817260805384, 0.0, 0.0, 1.0);

    D1 = (cv::Mat_<double>(1, 4) << -0.3448173534502492, 0.09834339989635991,
          0.0006913356388736054, -0.0001326767741732132);

    D2 = (cv::Mat_<double>(1, 4) << -0.3397900304621515, 0.095616923755259,
          0.0011394239795359034, -0.0010376635277968291);

    R = (cv::Mat_<double>(3, 3) << 1, 0, 0, 0, 1, 0, 0, 0, 1);

    T = (cv::Mat_<double>(3, 1) << -0.24262, 0, 0);
    // RCLCPP_INFO(this->get_logger(), "StereoRectify");
    cv::stereoRectify(K1, D1, K2, D2, cv::Size(640, 512), R, T, R1, R2, P1, P2,
                      Q);
    // RCLCPP_INFO(this->get_logger(), "StereoRectify");
    cv::initUndistortRectifyMap(K1, D1, R1, P1, cv::Size(640, 512), CV_32FC1,
                                leftMapX, leftMapY);
    // RCLCPP_INFO(this->get_logger(), "StereoRectify");
    cv::initUndistortRectifyMap(K2, D2, R2, P2, cv::Size(640, 512), CV_32FC1,
                                rightMapX, rightMapY);
    ...
    RCLCPP_INFO(this->get_logger(),
                "Created engine and allocated device buffers");
    // string subscription_topic = "/thermal_" + cam + "/image";
    left_subscriber.subscribe(this, "/thermal_left/image",
                              qos.get_rmw_qos_profile());
    right_subscriber.subscribe(this, "/thermal_right/image",
                               qos.get_rmw_qos_profile());
    ...
    sync->registerCallback(std::bind(
        &FoundationStereoDepthNode::stereoDepthCallback, this, _1, _2));
    disparity_publisher =
        this->create_publisher<sensor_msgs::msg::Image>("disparity", 10);
    points_publisher = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "foundation_points", 10);
  }

  void
  stereoDepthCallback(const sensor_msgs::msg::Image::ConstSharedPtr &left,
    ...
    //                        "SKIPPING FRAME WITH DT " << dt.seconds() << endl);
    //   } else {
    //     RCLCPP_INFO_STREAM(this->get_logger(),
    //                        "SKIPPING FRAME NOT STARTED" << endl);
    //   }
    //   return;
    // }
    ...
    const cv_bridge::CvImagePtr cv_left =
        cv_bridge::toCvCopy(left, left->encoding);
    const cv_bridge::CvImagePtr cv_right =
        cv_bridge::toCvCopy(right, right->encoding);
    // RCLCPP_INFO_STREAM(this->get_logger(), left->encoding);

    cv::Mat rectifiedLeft;
    cv::remap(cv_left->image, rectifiedLeft, leftMapX, leftMapY,
              cv::INTER_LINEAR);
    cv::Mat rectifiedLeftF32;
    rectifiedLeft.convertTo(rectifiedLeftF32, CV_32F);
    cv::Mat sortedLeft;
    cv::sort(rectifiedLeftF32.reshape(1, 1), sortedLeft,
    ...
    cv::threshold(rectifiedLeftF32, rectifiedLeftF32, lp99Left - lp1Left,
                  lp99Left - lp1Left, cv::THRESH_TRUNC);
    ...
    cv::remap(cv_right->image, rectifiedRight, rightMapX, rightMapY,
              cv::INTER_LINEAR);
    cv::Mat rectifiedRightF32;
    rectifiedRight.convertTo(rectifiedRightF32, CV_32F);
    cv::Mat sortedRight;
    cv::sort(rectifiedRightF32.reshape(1, 1), sortedRight,
             cv::SORT_EVERY_ROW + cv::SORT_ASCENDING);
    float32_t lp1Right =
        sortedRight.at<float32_t>(int(0.01 * sortedRight.cols));
    float32_t lp99Right =
        sortedRight.at<float32_t>(int(0.99 * sortedRight.cols));
    rectifiedRightF32 -= lp1Right;
    cv::threshold(rectifiedRightF32, rectifiedRightF32, lp99Right - lp1Right,
                  lp99Right - lp1Right, cv::THRESH_TRUNC);
    ...
    cudaMemcpy(disparity.data, disparity_buffer, 640 * 512 * 4,
    ...
    for (int v = 0; v < disparity.rows; ++v) {
      for (int u = 0; u < disparity.cols; ++u, ++iter_x, ++iter_y, ++iter_z) {
        ...
        // Set NaN for points with Z > 3.0m
        if (point[2] > 3.0f) {
    ...
    // auto avg_time = (left->header.stamp.nanosec +
    // right->header.stamp.nanosec) / 2;
    cv_bridge::CvImage disparity_message(left->header, "mono8",
                                         normalizedDisparity);
    sensor_msgs::msg::Image::SharedPtr disparityMessage =
        disparity_message.toImageMsg();
    disparityMessage->header.stamp = left->header.stamp;
    // disparity = K1[0, 0] * 0.24262 / disp
    RCLCPP_INFO_STREAM(this->get_logger(),
                       "Publishing, max_depth is " << max_depth);
